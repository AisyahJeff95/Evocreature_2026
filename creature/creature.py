...
class create:
  def __init__(self):
    self.HalfExtents={} #Stores the radius of all blocks
    self.input_coordinate={} #stores all input coord
    self.output_coordinate={} #stores all output coord
    self.partsType={} 
    self.visualType={}
    self.jointsType={}
    self.identification={}
    self.jointGlobalPosition=defaultdict(list)
  ...
  # this code, coordinate calculations
  # resetting state of object
  # 
  ...
  def get_jointPosition(self):
    for i in [j for j, key in self.identification.items() if key == 'box']:
      if self.jointGlobalPosition[i]==[]:
        ...
        self.jointGlobalPosition[i]=jointGlobalPosition
  ...
  def create_base(self,boxRadius,basePosition,baseOrientation,startPartsNum=1):
    ...
    for i in range(startPartsNum-1): #beginning, no parts.
      extraPartsNum=0
  ...
  def radar(self,targetId):
    radar=[0]*8 #initiates radar list with 8 elements, initially 0. 

    #position & orientation of self object
    ...
    # Absolute angle of own
    self_angle=p.getEulerFromQuaternion(selfOrientation)[-1]
    # Relative angles between themselves and their opponents
    # The angle at a joint formed between the longitudinal axes of the body segments adjacent to the joint.
    self_another_angle=np.arctan2(targetPosition[1]-selfPosition[1],targetPosition[0]-selfPosition[0])
    # The direction of the opponent from the direction in which you are facing.
    total_angle=np.mod(2*np.pi+self_another_angle-self_angle,2*np.pi)
    # Directional index of sound.
    orientation_index=np.array(total_angle//(2*np.pi/8),dtype=int) #range of turns.
