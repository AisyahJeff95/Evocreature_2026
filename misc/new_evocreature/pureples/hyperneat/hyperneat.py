...
def query_cppn(coord1, coord2, outgoing, cppn, max_weight=5.0):
    ...
    if outgoing:
        i = [*[coor for coor in coord1], *[coor for coor in coord2], 1.0, *7*[0]]
    else:
        i = [*[coor for coor in coord2], *[coor for coor in coord1], 1.0, *7*[0]]
    w = cppn.activate(i)[0] #出力ノードの1番目を重さとして使用している
    if abs(w) > 0.2:  # If abs(weight) is below threshold, treat weight as 0.0.
        if w > 0:
            w = (w - 0.2) / 0.8
        else:
